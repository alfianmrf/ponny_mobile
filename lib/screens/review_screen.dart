...
class _ReviewStateScreen extends State<ReviewScreen> {
  ...
  Future<bool> _fetchInsert(BuildContext contexts) async {
    ...
    var token =
        Provider.of<AppModel>(contexts, listen: false).auth.access_token;
    Map<String, String> headers = {"Authorization": "Bearer $token"};
    var request = http.MultipartRequest("POST", Uri.parse(revieStore));
    ...
    if (widget.orderReview != null) {
      request.fields[widget.orderReview.label] =
          widget.orderReview.id.toString();
    }
    ...
    request.fields["at_ponny"] = widget.orderReview != null
        ? "1"
        : _status
            ? "1"
            : "0";
  ...
  Future<bool> _validation(BuildContext contexts) async {
    bool result = false;
    if (_recomended != null &&
        _catatan.value.text.isNotEmpty &&
        _catatan.value.text.length >= 50 &&
        widget.orderReview != null) {
      result = await _fetchInsert(contexts);
    } else if (_recomended != null &&
        _catatan.value.text.isNotEmpty &&
        _catatan.value.text.length >= 50 &&
        _status != null) {
      result = await _fetchInsert(contexts);
      ...
      final snackBar = SnackBar(
        content: Text('Form Review belum lengkap!',
