...
class CartModel with ChangeNotifier {
  ...
  Future<CartResult> addProductToCart(
      Product product, String token, VarianResult variant) async {
    CartResult result;
    int index;
    if (variant != null)
      index = listCardOfitem.indexWhere((element) =>
          element.product.id == product.id &&
          element.variant == variant.varian);
    else
      index = listCardOfitem
    ...
    if (index < 0) {
      ...
      print(variant);
      param.addAll({"product_id": product.id, "qty": 1});
      print(param);
      final res = await http.post(addtocart,
          headers: {
      ...
      if (res.statusCode == 200) {
        ...
        if (variant != null) {
          param.addAll({"variant": variant.varian});
          listCardOfitem.add(Cart(
              jsonData["cart_id"], 1, product, variant.varian, variant.price));
        } else {
          listCardOfitem.add(Cart(jsonData["cart_id"], 1, product, null,
      ...
      param.addAll({"product_id": product.id, "qty": quantity + 1});
      if (variant != null) {
        param.addAll({"variant": variant.varian});
      }
  ...
  Future<void> addSampleToCart(Product product, String token) async {
    ...
    if (index < 0) {
      final res =
          await http.post(addSample + "/" + product.id.toString(), headers: {
        HttpHeaders.contentTypeHeader: 'application/json',
        HttpHeaders.authorizationHeader: "Bearer $token"
      });
      // print(res.body);
  ...
  Future<void> addRedemToCart(ProductPoin product, String token) async {
    int index =
        listProductPoin.indexWhere((element) => element.id == product.id);
    if (index < 0) {
      var param = {"qty": 1};
      final res =
          await http.post(rendemProduct + "/" + product.product.id.toString(),
              headers: {
                HttpHeaders.contentTypeHeader: 'application/json',
                HttpHeaders.authorizationHeader: "Bearer $token"
              },
  ...
  Future<void> RemoveProductToCart(
    ...
    int index = listCardOfitem.indexWhere((element) =>
        element.product.id == product.id && element.variant == variant);
    // print(index);
  ...
  Future<void> DeleteProductSample(Product product, String token) async {
    final res = await http
        .get(removeProductSample + "/" + product.id.toString(), headers: {
      HttpHeaders.contentTypeHeader: 'application/json',
      HttpHeaders.authorizationHeader: "Bearer $token"
    });
    // print(res.body);
    if (res.statusCode == 200) {
      int index =
          listUseSample.indexWhere((element) => element.id == product.id);
  ...
  Future<void> DeleteRendemProduct(ProductPoin product, String token) async {
    final res = await http
        .get(removePoinProduct + "/" + product.product.id.toString(), headers: {
      HttpHeaders.contentTypeHeader: 'application/json',
      HttpHeaders.authorizationHeader: "Bearer $token"
    });
    if (res.statusCode == 200) {
      ...
      if (result.statusCode == 200) {
        ...
        for (Map item in responseJson["productPoint_used"]) {
          if (item["product"] != null)
            listProductPoin.add(ProductPoin(
                item["id"],
                item["product_point"] != null
                    ? item["product_point"]["jml_point"]
                    : null,
                item["product"] != null
                    ? Product.fromJson(item["product"]["availability"])
                    : null,
                item["qty"]));
        }
  ...
  Future<void> getSummary(String token) async {
    ...
    if (_courier != null && _coupon != null) {
      param = {"courier": _courier, "coupon_code": _coupon};
    } else if (_courier != null) {
      param = {"courier": _courier};
    } else if (_coupon != null) {
      param = {"coupon_code": _coupon};
    }

    print(param);
    final res = await http.post(cartSummary,
        headers: {
          HttpHeaders.contentTypeHeader: 'application/json',
          HttpHeaders.authorizationHeader: "Bearer $token"
        },
  ...
  Future<void> getPointSummary(
      bool usingPoint, String available, String token) async {
    String _coupon;
    String _courier;
    var params = <String, dynamic>{"using_point": usingPoint};
  ...
  Future<bool> AppyCoupon(String code, String token) async {
    var param = {"code": code};
    final response = await http.post(applyCoupon,
        headers: {
          HttpHeaders.contentTypeHeader: 'application/json',
          HttpHeaders.authorizationHeader: "Bearer $token"
        },
  ...
  Future<OrderResult> NewCheckout(String token, Address useAddress,
      String method, int cabang_id, bool using_point, Datum dataCabang) async {
    OrderResult result;
    if (dataCabang != null) {
    ...
    if (using_point == true) param.addAll({"using_point": using_point});
...
class Summary {
  ...
  factory Summary.fromJson(Map<String, dynamic> parsedJson) {
    return Summary(
        ...
        parsedJson["shipping"],
        parsedJson["total"],
        parsedJson["earnPoint"],
        parsedJson["available_point_to_use"],
        parsedJson["using_point"]);
  }
}
